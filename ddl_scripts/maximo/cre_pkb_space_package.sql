...
FUNCTION GET_PROJECT_NAME(PROJECT_ID_I  IN  maximo.lbl_v_projact.glaccount%TYPE)
                         RETURN maximo.lbl_v_projact.accountname%TYPE
IS
     DESCR_O       MAXIMO.lbl_v_projact.accountname%TYPE;
BEGIN
     SELECT accountname into descr_o
     from maximo.lbl_v_projact
...
FUNCTION GET_LEVEL_DESC (LEVEL1_I  IN SPADM.SPACE_ROOM.ORG_LEVEL_1_CODE%TYPE,
                         ...
                         RETURN DW.ORG_DEPARTMENT.DESCR%TYPE
IS
DESCR_O    DW.ORG_DEPARTMENT.DESCR%TYPE;
DESCR_O_T  DW.ORG_DEPARTMENT.DESCR%TYPE;
BEGIN
      IF LEVEL_NUMBER_I = 1 THEN
          SELECT DESCR INTO DESCR_O_T FROM DW.ORG_DEPARTMENT
      ...
      ELSIF LEVEL_NUMBER_I = 2 THEN
          SELECT DESCR INTO DESCR_O_T FROM DW.ORG_GROUP
      ...
      ELSIF LEVEL_NUMBER_I = 3 THEN
          SELECT DESCR INTO DESCR_O_T FROM DW.ORG_UNIT
          WHERE DEPTID=RTRIM(LEVEL1_I)     AND
      ...
      ELSIF LEVEL_NUMBER_I = 4 THEN
          SELECT DESCR INTO DESCR_O_T FROM DW.ORG_LEVEL4
          WHERE DEPTID=RTRIM(LEVEL1_I)     AND
                ZZ_GROUP=RTRIM(LEVEL2_I)   AND
...
BEGIN
     ...
     -- FLOORS ARE NOT ADDED AUTOMATICALLY. THEREFORE COMMENTED THE SECTION 
     -- THAT ADDS THE FLOOR RECORDS  
...
BEGIN
   ...
   -- GET LOCATION CODE FOR MAXIMO TABLES
     ...
     -- NOW INSERT INTO OTHER LOCATION RELATED TABLES
    INSERT_LOCATION_TABLES(LOCATION_T, BUILDING_NUMBER_I, NULL, NULL, CHANGEBY_I);
...
FUNCTION GET_DEFAULT_PROJECT(LEVEL1_I   IN  SPADM.SPACE_ROOM.ORG_LEVEL_1_CODE%TYPE)
                             RETURN MAXIMO.lbl_v_projact.glaccount%TYPE
IS
    PROJECT_ID_O  MAXIMO.lbl_v_projact.glaccount%TYPE;
BEGIN
...
IS
   DEFAULT_PROJECT_ID_T MAXIMO.lbl_v_projact.glaccount%TYPE;
   REC_CNT_T    NUMBER(5);
...
FUNCTION GET_EMPLOYEE_NAME(EMPLOYEE_ID_I  IN DW.PEOPLE_CURRENT.EMPLOYEE_ID%TYPE)
                           RETURN DW.PEOPLE_CURRENT.EMPLOYEE_NAME%TYPE
IS

 EMPLOYEE_NAME_O DW.PEOPLE_CURRENT.EMPLOYEE_NAME%TYPE;

 BEGIN
   SELECT EMPLOYEE_NAME INTO EMPLOYEE_NAME_O
   FROM DW.PEOPLE_CURRENT
...
IS
  BEGIN
      ...
      OPEN OCC_RESULTSET FOR
        ...
        FROM DW.PEOPLE_CURRENT A, HRADM.PS_ZZ_WRK_LOCATION B
