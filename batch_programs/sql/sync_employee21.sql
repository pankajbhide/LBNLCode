/************************************************************************
*
* PROGRAM NAME          : SYNC_EMPLOYEE.SQL
*
*
...
DECLARE CURSOR EMP_CUR IS
 SELECT LTRIM(RTRIM(A.EMPLID)) EMPLID, A.NAME, INITCAP(A.FIRST_NAME) FIRST_NAME,
    ...
    --FROM DW.PS_EMPLOYEES_PUBLIC@DWPRD A, BL@ARCHIBUS21 B, RM@ARCHIBUS21  C
    FROM DW.PS_EMPLOYEES_PUBLIC A;
 ...
 FUNCTION IS_ORG_LEVEL_VALID(
    ORG_LEVEL_1_CODE_I IN DW.PS_EMPLOYEES_PUBLIC.ORG_LEVEL_1_CODE%TYPE,
    ORG_LEVEL_2_CODE_I IN DW.PS_EMPLOYEES_PUBLIC.ORG_LEVEL_2_CODE%TYPE,
    ORG_LEVEL_3_CODE_I IN DW.PS_EMPLOYEES_PUBLIC.ORG_LEVEL_3_CODE%TYPE,
    ORG_LEVEL_4_CODE_I IN DW.PS_EMPLOYEES_PUBLIC.ORG_LEVEL_4_CODE%TYPE,
    LEVEL_I IN NUMBER)
RETURN VARCHAR2
 IS
...
BEGIN
   ...
   IF (LEVEL_I=1) THEN
    ...
    IF (LEVEL_I=3) THEN
      BEGIN
        SELECT '1' INTO SUCCESS_T
        FROM LBL_ORG_LEVEL_3@ARCHIBUS21    A
        WHERE A.DV_ID=ORG_LEVEL_1_CODE_I
        AND   A.DP_ID=ORG_LEVEL_2_CODE_I
    ...
    IF (LEVEL_I=4) THEN
      BEGIN
        SELECT '1' INTO SUCCESS_T
        FROM LBL_ORG_LEVEL_4@ARCHIBUS21    A
        WHERE A.DV_ID=ORG_LEVEL_1_CODE_I
        AND   A.DP_ID=ORG_LEVEL_2_CODE_I
        AND   A.LBL_ORG_LEVEL_3_CODE=ORG_LEVEL_3_CODE_I
