/*****************************************************************************
 PROGRAM NAME           : SYNC_ORGSTRUCTURE4ARCH.SQL
 ...
 PURPOSE                : PROGRAM TO SYNCCHRONIZE THE ORGANIZATION STRUCTURE DATA
                         ...
                         REVISED FOR ARCHIBUS 23
 ******************************************************************************/
WHENEVER SQLERROR EXIT 1 ROLLBACK;

DECLARE

  CURSOR DEPT_CUR  IS SELECT DEPTID, SUBSTR(DESCR,1,25) DESCR FROM EDW_SHARE.ORG_DEPARTMENT;

  CURSOR GROUP_CUR IS SELECT DEPTID, ZZ_GROUP, SUBSTR(DESCR,1,25) DESCR FROM EDW_SHARE.ORG_GROUP
                      WHERE DEPTID IN (SELECT DISTINCT
                      DV_ID FROM DV@archibus23);

  CURSOR UNIT_CUR  IS SELECT DEPTID,ZZ_GROUP, ZZ_UNIT,SUBSTR(DESCR,1,25) DESCR FROM EDW_SHARE.ORG_UNIT
                      WHERE DEPTID || ZZ_GROUP IN
                      (SELECT DISTINCT DV_ID ||
                                       DP_ID
                                       FROM DP@archibus23);

  CURSOR LEVEL4_CUR IS SELECT DEPTID,ZZ_GROUP,ZZ_UNIT, ZZ_LEVEL4, SUBSTR(DESCR,1,25) DESCR FROM EDW_SHARE.ORG_LEVEL4
  ...
  FOR UNIT_REC IN UNIT_CUR
   ...
   LOOP
     ...
     BEGIN
      ...
      IF (T_ORG_LEVEL_3_DESCRIPTION !=
          UNIT_REC.DESCR)
           ...
           UPDATE LBL_ORG_LEVEL_3@archibus23
     ...
     EXCEPTION
      ...
      WHEN NO_DATA_FOUND THEN
           INSERT INTO LBL_ORG_LEVEL_3@archibus23
  ...
  FOR LEVEL4_REC IN LEVEL4_CUR
   ...
   LOOP
     ...
     BEGIN
      ...
      IF (T_ORG_LEVEL_4_DESCRIPTION !=
          LEVEL4_REC.DESCR)
           ...
           UPDATE LBL_ORG_LEVEL_4@archibus23
     ...
     EXCEPTION
      ...
      WHEN NO_DATA_FOUND THEN
           INSERT INTO LBL_ORG_LEVEL_4@archibus23
