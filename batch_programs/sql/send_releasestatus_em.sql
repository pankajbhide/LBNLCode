...
/***************************************************************************
*
* PROGRAM NAME          : send_releasestatus_em.SQL
*
*
* DESCRIPTION           : Send out Email when work order status change to REL, WREL, RFI.
*
*
...
CURSOR  UPDATEWOSTATUS_CUR IS
   ...
   FROM   WORKORDER 
   WHERE  ORGID = 'LBNL' and SITEID = 'FAC' and STATUS in ('RFI', 'REL', 'WREL') AND ISTASK = 0     
   AND STATUSDATE > (SELECT to_date(VARVALUE,'YYYY-MM-DD HH24:MI:SS') FROM LBL_MAXVARS WHERE  VARNAME='LAST_LOC_RELEASE_EMAIL_SEND');
 ...
 BEGIN  -- main program
   ...
   FOR UPDATEWOSTATUS_REC IN UPDATEWOSTATUS_CUR
     ...
     LOOP
        ...
        T_BODY2 := '<TABLE BORDER=1 ALIGN=LEFT>';  -- Pankaj 
        -- Pankaj Separated titles from its contents  
        ...
        IF UPDATEWOSTATUS_REC.STATUS in ('REL', 'RFI') THEN
     ...
     SEND_EMAIL(T_SUBJECT, T_BODY, T_TESTUSER_EMAIL, T_APP_ENVIRONMENT, UPDATEWOSTATUS_REC.SUPERVISOR, 
        UPDATEWOSTATUS_REC.LOCATION, UPDATEWOSTATUS_REC.STATUS);
   END LOOP;


   UPDATE LBL_MAXVARS
    SET VARVALUE = to_char(sysdate,'YYYY-MM-DD HH24:MI:SS')
    WHERE VARNAME='LAST_LOC_RELEASE_EMAIL_SEND';
    commit;
