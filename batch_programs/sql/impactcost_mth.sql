...
DECLARE
 ...
 CURSOR INPUT_RECORDS_CUR IS
    ...
    CURSOR LBL_MAXVARS_CUR IS
      SELECT VARVALUE FROM LBL_MAXVARS
...
BEGIN
     ...
     IF UPPER('&3')='ALL' THEN   -- START FROM OCTOBER (FIRST ACCOUNTING PERIOD) 
        SELECT FISCAL_YEAR, FISCAL_YEAR,
               1, PERIOD
        INTO FROM_FISCAL_YEAR_T, TO_FISCAL_YEAR_T, 
             FROM_ACCOUNTING_PERIOD_T, TO_ACCOUNTING_PERIOD_T      
        FROM DW_LBNL_HARD_CLOSE
        WHERE ROWNUM=1;
     ELSE 
       SELECT FISCAL_YEAR, FISCAL_YEAR, 
              PERIOD , PERIOD 
       INTO   FROM_FISCAL_YEAR_T, TO_FISCAL_YEAR_T, 
              FROM_ACCOUNTING_PERIOD_T, TO_ACCOUNTING_PERIOD_T      
       FROM DW_LBNL_HARD_CLOSE
       WHERE ROWNUM=1;   -- IT IS ASSUMED THAT THE TABLE CONTAINS ONLY ONE RECORD
    END IF;
...
--        AND   SITEID=SITEID_T;
    ...
    -- GET THE VALUE OF LAST FISCAL YEAR AND ACC PERIOD FOR WHICH ACTUALS HAVE 
    -- BEEN COPIED TO MAXIMO 
    
    FOR LBL_MAXVARS_REC IN  LBL_MAXVARS_CUR 
    
      LOOP
        COPIED_FISCAL_YEAR_T       :=  TO_NUMBER(SUBSTR(LBL_MAXVARS_REC.VARVALUE,1,4));
        COPIED_ACCOUNTING_PERIOD_T :=  TO_NUMBER(SUBSTR(LBL_MAXVARS_REC.VARVALUE,5,2));             
    END LOOP;         
    ...
    FOR INPUT_RECORDS_REC IN INPUT_RECORDS_CUR
      ...
      -- IF ACUTALS ARE ALREADY COPIED, THEN EXIT THE PROGRAM 
      IF (INPUT_RECORDS_REC.FISCAL_YEAR=COPIED_FISCAL_YEAR_T AND
          INPUT_RECORDS_REC.ACCOUNTING_PERIOD=COPIED_ACCOUNTING_PERIOD_T ) THEN
         EXIT;
      END IF;
